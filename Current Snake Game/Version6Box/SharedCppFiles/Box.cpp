...
1
	...
	namespace GameBoxes
	{
		...
		void Box::createBackGroundTexture(sf::Image & background, sf::Texture & texture)
		{
			background.create(windowWidth, windowHeight, sf::Color::Black);
			if (!texture.loadFromImage(background))
       		{
            	exit(EXIT_FAILURE);
       		}
		}
