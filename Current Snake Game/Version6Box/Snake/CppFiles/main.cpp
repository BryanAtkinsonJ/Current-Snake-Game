	...
	int main(int argc, char **argv)
	{
		...
		sf::Time timePerFrame = sf::seconds(1.0f / 45.0f); // 60 frames per second
		...
		sf::Font font;
		if (!font.loadFromFile("Version6Box/Snake/CppFiles/sansation.ttf"))
		{
			exit(1);	
		}
		...
		text.setCharacterSize(60 * cArgs[1] / ( 800));
//		text.setFillColor(sf::Color::Blue);
			
		text.setStyle(sf::Text::Bold | sf::Text::Underlined);
		text.setPosition(45.0f * cArgs[1] / (800), 200.0f * cArgs[1] / (800));
		...
		while (window.isOpen())
        {
				...
				// Eventually use this to move the snake
			{
				...
				if (count % 3 == 2)
				{
					(*(twoDGridVector[x_index][y_index])).setSquareType(-4);
				...
				if (count % 3 == 1)
				{
					...
					{
						(*(twoDGridVector[x_index][y_index])).setSquareType(0);
						window.draw(*(twoDGridVector[x_index][y_index])); // Draw a GridPiece
					}
					
					if (x_index % 2 == 1 && y_index % 2 == 1)
					{
						(*(twoDGridVector[x_index][y_index])).setSquareType(0);
						window.draw(*(twoDGridVector[x_index][y_index])); // Draw a GridPiece
					}
				...
				if (count % 3 == 0)
				{
					if (x_index % 4 == 0 & y_index % 4 == 0)
					{
						(*(twoDGridVector[x_index][y_index])).setSquareType(-1);
						window.draw(*(twoDGridVector[x_index][y_index])); // Draw a GridPiece
					}
					
					if (x_index % 4 == 1 && y_index % 4 == 1)
					{
						(*(twoDGridVector[x_index][y_index])).setSquareType(-3);
						window.draw(*(twoDGridVector[x_index][y_index])); // Draw a GridPiece
					}
				}
